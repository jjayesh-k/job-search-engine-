...
Flags:
  ...
  --notify-only       Step 4: write CSV + send Gmail (from cached scored_jobs.json)
  --fetch-and-score   Steps 2 + 3
  --no-llm            Skip Ollama summaries (faster, used in CI)
  (no flag)           Full pipeline: Steps 2 → 3 → CSV → Email
...
def _load_scored_jobs() -> list:
    """Load ScoredJob objects from outputs/scored_jobs.json."""
...
def run_fetch_only() -> list[dict]:
    """Fetch fresh jobs from Adzuna + Serpapi and save to outputs/raw_jobs.json."""
...
def run_score_only(use_llm: bool = True) -> list:
    """Score cached raw_jobs.json against resume. Saves outputs/scored_jobs.json."""
    ...
    # Load resume (PDF preferred, falls back to .txt if present)
...
def run_notify_only() -> bool:
    """
    Load scored_jobs.json, write dated CSV shortlist, send Gmail notification.
    ...
    print("\n" + "=" * 60)
    print("  STEP 4/5 — CSV Output + Gmail Notification")
    print("=" * 60 + "\n")
    ...
    print(f"  Jobs above threshold ({MIN_FIT_SCORE}): {len(shortlist)}\n")

    # Write dated CSV + shortlist_latest.csv
    csv_path, written = write_shortlist_csv(all_jobs)
    print(f"  CSV written : {csv_path} ({len(written)} jobs)")
    ...
    print(f"  Email       : {'SENT ✓' if ok else 'FAILED — check logs above'}")
    return ok
...
def run_full_pipeline(use_llm: bool = True) -> None:
    """Steps 2 → 3 → CSV → Email. The same flow GitHub Actions runs daily."""
    run_fetch_only()
...
Examples:
  ...
  python main.py --score-only                # Step 3 only (with Ollama)
  python main.py --score-only --no-llm       # Step 3 only (fast, no Ollama)
  python main.py --notify-only               # CSV + email from cached scores
  ...
  python main.py                             # Full pipeline with Ollama
    ...
    parser.add_argument(
        ...
        help="Step 2: fetch jobs from APIs",
    )
    parser.add_argument(
        "--score-only", action="store_true",
        help="Step 3: score cached raw_jobs.json",
    )
    parser.add_argument(
        "--notify-only", action="store_true",
        help="Step 4/5: write CSV + send email from cached scored_jobs.json",
    )
    parser.add_argument(
        "--fetch-and-score", action="store_true",
        help="Steps 2+3: fetch then score",
    )
    parser.add_argument(
        "--no-llm", action="store_true",
        help="Skip Ollama LLM summaries (faster, used in CI)",
    )
